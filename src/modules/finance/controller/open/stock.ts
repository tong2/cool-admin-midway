...
export class OpenFinanceStockController extends BaseController {
  ...
  async sqlPage(@Body() query) {
    return this.ok(await this.FinanceStockService.sqlPage(query));
  }

  @Post('/entityPage', { summary: 'entity分页查询' })
  async entityPage(@Body() query) {
    return this.ok(await this.FinanceStockService.entityPage(query));
  }
  ...
  async exportOrders() {
    try {
      // 从数据库中获取订单数据
      const orders = await this.FinanceStockEntity.find();
