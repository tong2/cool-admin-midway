...
export class OpenFinanceTalentCommissionController extends BaseController {
  ...
  async sqlPage(@Body() query) {
    return this.ok(await this.FinanceTalentCommissionService.sqlPage(query));
  }

  @Post('/entityPage', { summary: 'entity分页查询' })
  async entityPage(@Body() query) {
    return this.ok(await this.FinanceTalentCommissionService.entityPage(query));
  }
  ...
  async exportOrders() {
    try {
      // 从数据库中获取订单数据
      const orders = await this.FinanceTalentCommissionEntity.find();
    ...
    try {
      ...
      for (const sheet of workSheets) {
        ...
        for (let i = 1; i < rows.length; i++) {
          const row = rows[i];
          await this.FinanceTalentCommissionEntity.save({
