...
import { FinanceBoxSpecService } from '../../service/box_spec';
import { FinanceBoxSpecQueryDTO } from '../../dto/box_spec';
...
@CoolController({
  ...
  entity: FinanceBoxSpecEntity,
  service: FinanceBoxSpecService,
})
export class FinanceBoxSpecController extends BaseController {
  ...
  @Inject()
  financeBoxSpecService: FinanceBoxSpecService;

  /**
   * Export box specification records to Excel
   */
  @Get('/export')
  @Validate()
  async export(@Query() query: FinanceBoxSpecQueryDTO) {
    const { headers, data } = await this.financeBoxSpecService.export(query);

    // Create Excel workbook
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet('箱规表');
    ...
    const filename = encodeURIComponent('箱规表.xlsx');
  ...
  /**
   * Import box specification records from Excel
   */
  @Post('/import')
  async importExcel(@Files() files) {
    try {
      ...
      const result = await this.financeBoxSpecService.import(data);
  ...
  /**
   * Get paginated box specification records with alternative response format
   */
  @Post('/page2')
  async page2(@Body() query: FinanceBoxSpecQueryDTO) {
    try {
      const result = await this.financeBoxSpecService.list(query);
      return this.ok({
        list: result.list,
        pagination: {
          page: query.page,
