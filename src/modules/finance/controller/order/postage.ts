...
import { FinancePostageService } from '../../service/postage';
import { FinancePostageQueryDTO } from '../../dto/postage';
...
@CoolController({
  ...
  entity: FinancePostageEntity,
  service: FinancePostageService,
})
export class FinancePostageController extends BaseController {
  ...
  @Inject()
  financePostageService: FinancePostageService;

  /**
   * Export postal rate records to Excel
   */
  @Get('/export')
  @Validate()
  async export(@Query() query: FinancePostageQueryDTO) {
    const { headers, data } = await this.financePostageService.export(query);

    // Create Excel workbook
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet('邮资表');
    ...
    const filename = encodeURIComponent('邮资表.xlsx');
  ...
  /**
   * Import postal rate records from Excel
   */
  @Post('/import')
  async importExcel(@Files() files) {
    try {
      ...
      const result = await this.financePostageService.import(data);
  ...
  /**
   * Get paginated postal rate records with alternative response format
   */
  @Post('/page2')
  async page2(@Body() query: FinancePostageQueryDTO) {
    try {
      const result = await this.financePostageService.list(query);
      return this.ok({
        list: result.list,
        pagination: {
          page: query.page,
